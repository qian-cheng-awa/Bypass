...
local function updataitem()
    local tb = {}
    for i,v in ipairs(workspace:WaitForChild("RuntimeItems"):GetChildren()) do
        tb[i] = tostring(i).." "..tostring(v.Name)
    end
    return tb
end
...
local Dropdown = Tab:CreateDropdown({
    ...
    Callback = function(Options)
        ...
        item = workspace:WaitForChild("RuntimeItems"):GetChildren()[tonumber(string.sub(unpack(Options),0,string.find(unpack(Options)," ")))]
    end,
})
